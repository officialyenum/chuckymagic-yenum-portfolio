...
<div class="col-md-4 col-xl-3">
    ...
    <h6 class="sidebar-title">Event Categories</h6>
    <div class="row link-color-default fs-14 lh-24">
        @foreach ($categories as $category)
    <div class="col-6"><a href="{{route('dashboard.category', $category->id )}}">{{ $category->name }}</a></div>
        @endforeach
    </div>
    ...
    <h6 class="sidebar-title">Activities</h6>
    <div class="gap-multiline-items-1">
        @foreach ($tags as $tag)
            <a class="badge badge-secondary" href="{{route('dashboard.tag', $tag->id )}}">{{$tag->name}}</a>
        @endforeach
    </div>
